    ...
    try:
        ...
        if all_match:
            ...
            if current_chrome_version != "":
                ...
                current_chrome_version = ".".join(
                    current_chrome_version.split(".")[:-1]
                )
                version_match = [
                    i for i in all_match if re.search("^%s" % current_chrome_version, i)
                ]
            ...
            with zipfile.ZipFile(file_path, "r") as zip_ref:
                zip_ref.extractall(
                    os.path.normpath(os.path.join(app_path, "webdriver"))
                )
